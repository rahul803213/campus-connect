...
const register = async (req, res) => {
   ...
   const {username,email,password,phone_number,college_name,university_name,roll_number,
      biography, branch,session,registration_number}= req.body;
     // const profileImage = req.body.profile_picture;
...
user.save().then(()=>emailVerification.sendVerificationEmail(user.contact_details.Email, user.VerificationToken))
.then(()=>{
  const token = jwt.sign(payload(user),process.env.jwt_secret_key);
  return res.json({user_data:UserUtilities.UserModel(user,token),email_response:{message:"verification email sent"}});
})
...
//}
  ...
  const login = async(req,res) => {
    try{
      ...
      const user = await User.findOne({'contact_details.Email':normalizedEmail});
  ...
  const email_verifier= async (req, res) => {
   ...
   try {
    const user_update=  await User.findOneAndUpdate({VerificationToken:token},{isVerified:true,VerificationToken:null},{new:true})
    //user.verificationToken = null; // Remove the token to prevent reuse
   const result = await user_update.json();
    return res.status(200).json({ message: 'Email verified successfully',result:result });
  }
  catch(error){
